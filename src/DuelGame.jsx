...
import { Users, Copy, Check, LogOut, Gem } from "lucide-react";
...
export default function DuelGame({ onExit }) {
  ...
  async function startGame() {
    if (!roomId) return;
    await updateDoc(doc(db, "rooms", roomId), { phase: "placing" });
  }
      ...
      <div className="p-4 max-w-md mx-auto">
        ...
        {uid && screen === "lobby" && room && room.phase !== "lobby" && (
          <div className="mt-6 text-center text-sm text-[#9A94A8]">
            Partie lancée — la pose des pièces et le tour de jeu arrivent dans la prochaine étape 🙂
          </div>
        )}
      </div>
    </div>
