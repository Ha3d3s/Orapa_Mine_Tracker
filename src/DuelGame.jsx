...
import { requiredPieceTypes, COLS, ROWS, CELL, X0, Y0, VBW, VBH, BW, BH, PIECE_DEFS, effSize } from "./orapaEngine";
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-base font-semibold">
                {room.turn === uid ? "À toi de jouer" : "Tour de l'adversaire"}
              </h3>
              <button
                onClick={() => setReviewingMyBoard((v) => !v)}
                className="text-xs bg-[#232336] hover:bg-[#2E2E46] px-3 py-1.5 rounded-full"
              >
            ...
            {reviewingMyBoard ? (
              ...
              <div className="text-center text-sm text-[#9A94A8] py-10">
                Le plateau adverse et les actions (interroger / proposer / passer) arrivent dans la prochaine étape 🙂
              </div>
            )}
            <button onClick={leaveRoom} className="text-xs text-[#6B6580] hover:text-[#E88] mx-auto block mt-4">Quitter la partie</button>
          </div>
        )}
      </div>
    </div>
