...
import { Users, Copy, Check, LogOut, Gem, CheckCircle2, Trophy, SkullIcon } from "lucide-react";
...
export default function DuelGame({ onExit }) {
  ...
  const [reviewingMyBoard, setReviewingMyBoard] = useState(false);

  // ---- phase de jeu ----
  const [actionMode, setActionMode] = useState(null); // null | 'beam' | 'cell' | 'guess'
  ...
  const [myQueries, setMyQueries] = useState([]); // mes questions posées à l'adversaire (by === uid)
  const [activeQueryId, setActiveQueryId] = useState(null);
  const answeringRef = useRef(new Set()); // évite de répondre deux fois à la même question entrante
  const guessProcessedRef = useRef(new Set()); // évite de traiter deux fois le résultat d'une proposition
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(qRef, (snap) => {
      snap.docs.forEach((d) => {
        ...
        if (data.type === "beam") {
          ...
          answer = found ? { occupied: true, color: found.color, label: found.label } : { occupied: false };
    ...
    return () => unsub();
    // myPieces est dans les deps : on veut se réabonner avec la dernière version de mon
    // plateau réel dès qu'il change, pour toujours répondre avec des données à jour.
  }, [roomId, uid, myPieces]);

  // ---- écoute de MES questions (posées par moi) ----
  useEffect(() => {
    if (!roomId || !uid) return;
    const qRef = query(collection(db, "rooms", roomId, "queries"), where("by", "==", uid));
    const unsub = onSnapshot(qRef, (snap) => {
      setMyQueries(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    });
    return () => unsub();
  }, [roomId, uid]);
  ...
  // ---- traitement automatique du résultat d'une proposition ----
  useEffect(() => {
  ...
  // ---- révélation des plateaux une fois la partie terminée ----
  useEffect(() => {
    if (!roomId || !uid || !room) return;
  ...
  // historique (mes questions répondues, hors proposition) pour colorer le plateau adverse connu
    ...
    <div className="min-h-screen w-full bg-[#12121C] text-[#EDE9E0] font-sans">
      ...
      <div className="p-4 max-w-md mx-auto">
            ...
            <div>
              ...
              <div className="mt-4 flex flex-col items-center gap-2 pb-8">
                ...
                <p className="text-xs text-[#6B6580]">
                  Adversaire : {oReady ? <span className="text-[#5FBF6B]">prêt ✓</span> : "en train de placer ses pièces…"}
                </p>
        ...
        {uid && screen === "lobby" && room && room.phase === "playing" && (() => {
          ...
          const waitingForAnswer = activeQuery && activeQuery.status === "pending";
          const answeredNotEnded = activeQuery && activeQuery.status === "answered";

          return (
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-base font-semibold">{myTurn ? "À toi de jouer" : "Tour de l'adversaire"}</h3>
                <button onClick={() => setReviewingMyBoard((v) => !v)} className="text-xs bg-[#232336] hover:bg-[#2E2E46] px-3 py-1.5 rounded-full">
                  {reviewingMyBoard ? "Revenir au jeu" : "Revoir ma pose"}
                </button>
              </div>
              ...
              {reviewingMyBoard ? (
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-semibold">Reconstitue le plateau adverse</h4>
                  ...
                  <div className="flex gap-2 mt-3 pb-8">
                    <button onClick={() => setActionMode(null)} className="flex-1 py-2.5 rounded-xl bg-[#232336] hover:bg-[#2E2E46] text-sm">Annuler</button>
                    <button
                      onClick={submitGuess}
                      disabled={guessPieces.length !== allowedTypes.length}
                      className="flex-1 py-2.5 rounded-xl bg-[#F2C744] hover:bg-[#E0B62F] text-[#12121C] font-semibold disabled:opacity-40"
                    >
                ...
                <>
                  <OpponentBoard
                    interactive={myTurn && !activeQueryId && (actionMode === "beam" || actionMode === "cell")}
                    mode={actionMode}
                  ...
                  <div className="mt-4 flex flex-col items-center gap-3 pb-8">
                    {waitingForAnswer && (
                      <p className="text-sm text-[#F2C744] animate-pulse">En attente de la réponse de l'adversaire…</p>
                    )}

                    {answeredNotEnded && room.phase === "playing" && (
                      <div className="w-full bg-[#1B1B29] border border-[#2A2A3A] rounded-xl p-3 text-sm">
                        <QueryResult q={activeQuery} />
                        <button onClick={endMyTurn} className="w-full mt-3 py-2 rounded-lg bg-[#F2C744] hover:bg-[#E0B62F] text-[#12121C] font-semibold text-sm">
                          Terminer mon tour
                        </button>
                      </div>
                    )}

                    {myTurn && !activeQueryId && actionMode === null && (
                      <div className="w-full flex flex-col gap-2">
                        <button onClick={() => setActionMode("beam")} className="w-full py-3 rounded-xl bg-[#232336] hover:bg-[#2E2E46] text-sm font-medium">
                          Interroger un point d'entrée
                        </button>
                        <button onClick={() => setActionMode("cell")} className="w-full py-3 rounded-xl bg-[#232336] hover:bg-[#2E2E46] text-sm font-medium">
                          Interroger une case
                        </button>
                        <button
                          onClick={() => { setGuessPieces([]); setActionMode("guess"); }}
                          disabled={myGuessesLeft <= 0}
                          className="w-full py-3 rounded-xl bg-[#2E2650] hover:bg-[#3A2F66] text-sm font-medium disabled:opacity-40"
                        >
                          Faire une proposition ({myGuessesLeft} tentative{myGuessesLeft > 1 ? "s" : ""} restante{myGuessesLeft > 1 ? "s" : ""})
                        </button>
                        <button onClick={passTurn} className="w-full py-2.5 rounded-xl bg-[#1B1B29] border border-[#2A2A3A] hover:bg-[#232336] text-xs text-[#9A94A8]">
                          Passer mon tour
                        </button>
                      </div>
                    )}

                    {(actionMode === "beam" || actionMode === "cell") && !activeQueryId && (
                      <button onClick={() => setActionMode(null)} className="text-xs text-[#6B6580] hover:text-[#EDE9E0]">← Retour aux actions</button>
                    )}
                  </div>

                  {/* historique complet de mes questions */}
                    ...
                    <div className="mb-6">
                      ...
                      <ul className="flex flex-col gap-1.5">
                        {[...beamHistory, ...cellHistory].map((q) => (
                          <li key={q.id} className="text-xs bg-[#1B1B29] border border-[#2A2A3A] rounded-lg px-3 py-2">
                            <QueryResult q={q} compact />
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              {confirmBeamPort && (
                <ConfirmModal
                  text={`Interroger l'entrée ${confirmBeamPort.label} ?`}
                  onCancel={() => setConfirmBeamPort(null)}
                  onConfirm={() => submitBeamQuery(confirmBeamPort)}
                />
              )}
              {confirmCell && (
                <ConfirmModal
                  text={`Interroger la case (col ${confirmCell.col + 1}, ligne ${confirmCell.row + 1}) ?`}
                  onCancel={() => setConfirmCell(null)}
                  onConfirm={() => submitCellQuery(confirmCell.col, confirmCell.row)}
                />
              )}
            </div>
          );
        ...
        {uid && screen === "lobby" && room && room.phase === "ended" && (
          <div className="flex flex-col items-center gap-6 py-6">
            {room.winner === uid ? (
              <div className="flex flex-col items-center gap-2 text-[#F2C744]">
                <Trophy size={48} />
                <h2 className="text-xl font-bold">Tu as gagné ! 🎉</h2>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-2 text-[#9A94A8]">
                <SkullIcon size={48} />
                <h2 className="text-xl font-bold">Tu as perdu</h2>
              </div>
            )}

            <div className="w-full">
              <h3 className="text-sm font-semibold mb-2 text-center">Ton plateau</h3>
              <ReadOnlyBoard pieces={myPieces} />
            </div>

            <div className="w-full">
              <h3 className="text-sm font-semibold mb-2 text-center">Plateau adverse</h3>
              {room.finalBoards && room.finalBoards[oppUid] ? (
                <ReadOnlyBoard pieces={room.finalBoards[oppUid]} />
              ) : (
                <p className="text-center text-xs text-[#6B6580]">En attente de la révélation…</p>
              )}
            </div>

            <button onClick={leaveRoom} className="w-full py-3 rounded-xl bg-[#F2C744] hover:bg-[#E0B62F] text-[#12121C] font-semibold">
              Retour à l'accueil
            </button>
          </div>
        )}
...
function QueryResult({ q, compact }) {
  if (q.type === "beam") {
    ...
    if (q.answer.absorbed) return <span>Entrée {entryPort?.label} → <strong>absorbé</strong> par un corps noir</span>;
    ...
    return (
      <span className="flex items-center gap-2">
        ...
        Entrée {entryPort?.label} → sort en {q.answer.exitLabel} · {c.name}
      </span>
    );
  }
  if (q.type === "cell") {
    ...
    return q.answer.occupied ? (
      <span className="flex items-center gap-2">
        <span className="w-3 h-3 rounded-full border border-[#0C0C14] shrink-0" style={{ background: q.answer.color }} />
        Case ({label}) → {q.answer.label}
      </span>
    ) : <span>Case ({label}) → vide</span>;
  }
  if (q.type === "guess") {
...
// Plateau adverse : ports/cases colorés selon ce que j'ai appris, cliquables selon le mode actif.
function OpponentBoard({ interactive, mode, beamHistory, cellHistory, onPortTap, onCellTap }) {
  ...
  beamHistory.forEach((q) => {
    ...
    portInfo[entryPort.id] = { hex };
    if (exitPort) portInfo[exitPort.id] = { hex };
  });
  const cellInfo = {};
  cellHistory.forEach((q) => {
    cellInfo[q.params.col + "," + q.params.row] = q.answer.occupied ? { hex: q.answer.color } : { empty: true };
  });
  ...
  return (
    <svg viewBox={`0 0 ${VBW} ${VBH}`} className="w-full max-w-md mx-auto block" style={{ background: "#1B1B29", borderRadius: 10 }}>
      ...
      {/* cases connues */}
      {mode === "cell" || Object.keys(cellInfo).length > 0
              ...
              return (
                <g key={c + "-" + r}>
                  <rect
                    ...
                    fill={info ? (info.empty ? "#2A2A3F" : info.hex) : "transparent"}
                    opacity={info ? (info.empty ? 0.4 : 0.85) : 1}
                    stroke={mode === "cell" && interactive ? "#3A3A52" : "none"}
                    strokeWidth={0.5}
...
// Affichage seul (non interactif) du plateau du joueur — pour revoir sa propre pose en jeu.
      ...
      {(pieces || []).map((p) => {
        ...
        return (
          <g key={p.id} transform={transform}>
            <polygon points={ptsStr} fill={def.color} stroke={def.stroke || "#0C0C14"} strokeWidth={1.2} />
          </g>
        );
      })}
    </svg>
