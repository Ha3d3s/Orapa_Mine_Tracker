...
export default function DuelGame({ onExit }) {
  ...
  // statistiques locales — une seule fois par partie, pour les deux joueurs
  useEffect(() => {
    if (!room || room.phase !== "ended" || !uid || statsRecordedRef.current) return;
    statsRecordedRef.current = true;
    recordDuelResult(room.winner === uid);
  }, [room, uid]);
  ...
  function leaveRoom() {
    ...
    answeringRef.current = new Set(); guessProcessedRef.current = new Set(); statsRecordedRef.current = false;
  }
