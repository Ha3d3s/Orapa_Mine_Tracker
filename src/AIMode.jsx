...
export default function AIMode({ onExit }) {
  const [screen, setScreen] = useState("settings"); // 'settings' | 'play' | 'ended'
  const [guessAttempts, setGuessAttempts] = useState(3);
  ...
  const [allowedTypes, setAllowedTypes] = useState(BASE_TYPES);
  const [guessesLeft, setGuessesLeft] = useState(3);
  const [guessPieces, setGuessPieces] = useState([]);
  const [marks, setMarks] = useState(() => new Set());
  const [history, setHistory] = useState([]); // {type:'beam'|'cell', params, answer}
  ...
  function submitGuess() {
    ...
    if (correct) { setWon(true); setScreen("ended"); return; }
    const left = guessesLeft - 1;
    setGuessesLeft(left);
    if (left <= 0) { setWon(false); setScreen("ended"); }
  }
