...
export async function submitDailyScore(dateStr, seconds, name) {
  const user = await ensureSignedIn();
  const ref = doc(db, "dailyScores", `${dateStr}_${user.uid}`);
  const existing = await getDoc(ref);
  if (existing.exists() && existing.data().seconds <= seconds) return false;
  await setDoc(ref, { dateStr, uid: user.uid, name: (name || "Anonyme").slice(0, 24), seconds, ts: Date.now() });
  return true;
}

export async function fetchTopDailyScores(dateStr, limitN = 10) {
  const q = query(
