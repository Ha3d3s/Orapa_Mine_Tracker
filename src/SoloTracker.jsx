...
import { playShot, playAbsorbed } from "./sounds";
...
const SOLO_STORAGE_KEY = "orapa_solo_state_v1";
function loadSoloSave() {
  try { return JSON.parse(localStorage.getItem(SOLO_STORAGE_KEY) || "null"); } catch { return null; }
}

export default function SoloTracker({ onExit }) {
  const savedRef = useRef(loadSoloSave());
  const [pieces, setPieces] = useState(() => savedRef.current?.pieces || []);
  const [selectedId, setSelectedId] = useState(null);
  const [shots, setShots] = useState(() => savedRef.current?.shots || []);
  ...
  const [ext, setExt] = useState(() => savedRef.current?.ext || { diamant: false, corpsNoir: false });
  const [marks, setMarks] = useState(() => new Set(savedRef.current?.marks || [])); // cells marked "vide" ("c,r" keys)
  ...
  // sauvegarde locale automatique — reprend exactement où tu en étais après un rechargement
  useEffect(() => {
    const data = { pieces, shots, marks: [...marks], ext };
    localStorage.setItem(SOLO_STORAGE_KEY, JSON.stringify(data));
  }, [pieces, shots, marks, ext]);
  ...
  useEffect(() => {
    ...
    function move(e) {
      ...
      if (Math.hypot(e.clientX - dragInfo.startX, e.clientY - dragInfo.startY) > 6) movedRef.current = true;
    ...
    function up(e) {
      ...
      if (dragInfo.kind === "move") {
        setSelectedId(movedRef.current ? null : dragInfo.id);
      }
      setDragInfo(null);
      setDragPos(null);
      setPreviewCell(null);
  ...
  function startNewDrag(type, e) {
    ...
    setSelectedId(null);
    setDragPos({ x: e.clientX, y: e.clientY });
    setDragInfo({ kind: "new", type, rot: 0, flipH: false, flipV: false, startX: e.clientX, startY: e.clientY });
  }
  function startMoveDrag(piece, e) {
    ...
    setDragPos({ x: e.clientX, y: e.clientY });
    setDragInfo({ kind: "move", id: piece.id, type: piece.type, rot: piece.rot, flipH: piece.flipH, flipV: piece.flipV, startX: e.clientX, startY: e.clientY });
  }
      ...
      <div className="px-2 pt-3">
        ...
        >
          ...
          {pieces.map((p) => {
            ...
            return (
              <g key={p.id} data-piece-id={p.id} transform={transform} onPointerDown={(e) => startMoveDrag(p, e)} style={{ cursor: "grab", touchAction: "none" }}>
                <polygon
                  points={ptsStr}
                  fill={def.color}
