...
function portXY(p) {
  switch (p.side) {
    ...
    case "bottom": return { x: X0 + BW - (p.index + 0.5) * CELL, y: Y0 + BH + 18 };
    case "left": return { x: X0 - 18, y: Y0 + BH - (p.index + 0.5) * CELL };
...
const COLORS = [
  ...
  { id: "miss", name: "Aucun contact", common: "transparent", hex: "#E7E5E9" },
];
...
export default function OrapaTracker() {
  ...
  const [drag, setDrag] = useState(null); // {kind:'new'|'move', type, id?, rot, x, y}
  ...
  useEffect(() => {
    ...
    function up(e) {
      ...
      setDrag((d) => {
        ...
        const pt = clientToBoardPoint(svg, e.clientX, e.clientY);
        const def = PIECE_DEFS[d.type];
        const { w, h } = effSize(def, d.rot || 0);
        const colf = (pt.x - X0) / CELL - w / 2;
        const rowf = (pt.y - Y0) / CELL - h / 2;
        const col = Math.round(colf), row = Math.round(rowf);
        if (d.kind === "new") {
          if (isValid(d.type, col, row, 0, null)) {
            setPieces((ps) => [...ps, { id: uid(), type: d.type, col, row, rot: 0, flipH: false, flipV: false }]);
          }
        } else if (d.kind === "move") {
          if (isValid(d.type, col, row, d.rot, d.id)) {
            setPieces((ps) => ps.map((p) => (p.id === d.id ? { ...p, col, row } : p)));
          }
        }
        return null;
      });
  ...
  function startNewDrag(type, e) {
    ...
    setSelectedId(null);
    setDrag({ kind: "new", type, rot: 0, x: e.clientX, y: e.clientY });
  }
  function startMoveDrag(piece, e) {
    ...
    setDrag({ kind: "move", id: piece.id, type: piece.type, rot: piece.rot, x: e.clientX, y: e.clientY });
  }
  ...
  function onPortClick(port) {
    if (picking) return;
    if (!pendingEntry) { setPendingEntry(port); return; }
  ...
  function resetAll() {
    ...
    setPieces([]); setShots([]); setPendingEntry(null); setPendingExit(null); setPicking(false); setSelectedId(null);
  }
  ...
  return (
    <div className="min-h-screen w-full bg-[#12121C] text-[#EDE9E0] font-sans select-none" style={{ touchAction: "none" }}>
      ...
      {drag && (
        ...
        >
          <PieceIcon def={PIECE_DEFS[drag.type]} />
        </div>
      )}
