...
export default function App() {
  ...
  if (mode === "duel") return <ErrorBoundary onReset={() => setMode(null)}><DuelGame onExit={() => setMode(null)} /></ErrorBoundary>;
  if (mode === "puzzle") return <ErrorBoundary onReset={() => setMode(null)}><PuzzleMode onExit={() => setMode(null)} /></ErrorBoundary>;
  if (mode === "stats") return <ErrorBoundary onReset={() => setMode(null)}><StatsScreen onExit={() => setMode(null)} /></ErrorBoundary>;
      ...
      <div className="w-full max-w-xs flex flex-col gap-3">
        ...
        <button onClick={() => setMode("puzzle")} className="flex items-center gap-3 w-full bg-[#1B1B29] hover:bg-[#232336] border border-[#2A2A3A] rounded-2xl px-4 py-4 text-left transition-colors">
          ...
          <div>
            ...
            <div className="text-xs text-[#9A94A8]">100 grilles chronométrées, du plus simple au plus dur</div>
          </div>
        </button>
        ...
        <button onClick={() => setMode("stats")} className="flex items-center gap-2 justify-center text-xs text-[#6B6580] hover:text-[#EDE9E0] mt-1">
          <BarChart3 size={14} /> Mes statistiques
        </button>
        <button onClick={() => setShowTutorial(true)} className="flex items-center gap-2 justify-center text-xs text-[#6B6580] hover:text-[#EDE9E0]">
          <BookOpen size={14} /> Comment jouer ?
        </button>
      </div>
