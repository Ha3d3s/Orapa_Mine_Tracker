...
// "qu'y a-t-il en H4 ?" — révèle la couleur d'affichage de la pièce si elle touche cette case.
export function pieceAtCell(pieces, col, row) {
  for (const p of pieces) {
    const cells = touchedCells(p.type, p.col, p.row, p.rot, p.flipH, p.flipV);
    if (cells.includes(col + "," + row)) return { type: p.type, color: PIECE_DEFS[p.type].color, label: PIECE_DEFS[p.type].label };
  }
  return null;
}
...
export function boardsMatch(realPieces, guessPieces) {
  ...
  for (const rp of realPieces) {
    ...
    for (let i = 0; i < guessPieces.length; i++) {
      ...
      if (
        gp.type === rp.type && gp.col === rp.col && gp.row === rp.row &&
        gp.rot === rp.rot && !!gp.flipH === !!rp.flipH && !!gp.flipV === !!rp.flipV
      ) { found = i; break; }
    }
